...
Use `claude`, `cursor`, or `vscode` instead of `codex` for another host. The
setup command shows the planned change, creates a timestamped backup before
editing an existing file, is idempotent, and never writes the API key. Remove
only an entry managed by Arbitype with:
...
2. **Decision stability:** repeated Jev calls report selected-decision
   ...
   decision remains consistent. The observability boundary for future metadata
